...
- [3. Experimental Premises](#3-experimental-premises)
  ...
  - [3.3 A single base model, a single scale; the task domain has grown from one to two](#33-a-single-base-model-a-single-scale-the-task-domain-has-grown-from-one-to-two)
...
- [6a. Downstream Harm Ranking: Which Noise Type Hurts Training Outcomes the Most](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most)
  - [6a.1 Real Downstream Impact: Does Noise Actually Drag Down Model Performance](#6a1-real-downstream-impact-does-noise-actually-drag-down-model-performance)
  - [6a.2 Closed-Loop Cleaning as Corroborating Evidence for Harm Ranking](#6a2-closed-loop-cleaning-as-corroborating-evidence-for-harm-ranking)
  - [6a.3 triviaqa-ratio10 (QA task): under a homogeneous single task, harm is no longer diluted by the "7-benchmark average"](#6a3-triviaqa-ratio10-qa-task-under-a-homogeneous-single-task-harm-is-no-longer-diluted-by-the-7-benchmark-average)
- [6b. Feature Signatures of Each Noise Type: How Training Dynamics and Text Features Vary by Type](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type)
  - [6b.1 Landscape of Detection Difficulty: What's Visible, What Isn't](#6b1-landscape-of-detection-difficulty-whats-visible-what-isnt)
  - [6b.2 Cross-Noise-Type Transfer: Can Detectors Generalize](#6b2-cross-noise-type-transfer-can-detectors-generalize)
  - [6b.3 Cross-Ratio Transfer: Almost No Loss](#6b3-cross-ratio-transfer-almost-no-loss)
  - [6b.4 The Direction-Reversal Trap: Memorized Noise Needs a "Signed" Rule](#6b4-the-direction-reversal-trap-memorized-noise-needs-a-signed-rule)
  - [6b.5 Early Detection: Usable Signal Before Training Even Finishes](#6b5-early-detection-usable-signal-before-training-even-finishes)
  - [6b.6 Feature Attribution: What Is the Detector Actually Looking At](#6b6-feature-attribution-what-is-the-detector-actually-looking-at)
  - [6b.7 How Much Overlap Is There Among These Metrics?](#6b7-how-much-overlap-is-there-among-these-metrics)
  - [6b.8 Single-Feature Leave-One-Out: Which Metric Is Truly Irreplaceable?](#6b8-single-feature-leave-one-out-which-metric-is-truly-irreplaceable)
  - [6b.9 External Baseline Comparison: This Project's Training-Dynamics Detector vs. EL2N/GraNd/TracIn Proxies From the Literature](#6b9-external-baseline-comparison-this-projects-training-dynamics-detector-vs-el2ngrandtracin-proxies-from-the-literature)
  - [6b.10 How Much of the Wild-Noise Detection Signal Is a Response-Length Confound?](#6b10-how-much-of-the-wild-noise-detection-signal-is-a-response-length-confound)
- [6c. Label-Free Separability: Precision, Boundaries, and Real Benefit](#6c-label-free-separability-precision-boundaries-and-real-benefit)
  - [6c.1 Cleaning-Precision Lift: A High AUC Doesn't Guarantee a Useful Removal Budget](#6c1-cleaning-precision-lift-a-high-auc-doesnt-guarantee-a-useful-removal-budget)
  - [6c.2 Best Method Per Noise Type: A Current-State Summary](#6c2-best-method-per-noise-type-a-current-state-summary)
  - [6c.3 Feature Ablation: Which Data Is Necessary, Which Metric Is Irreplaceable?](#6c3-feature-ablation-which-data-is-necessary-which-metric-is-irreplaceable)
  - [6c.4 Boundaries of the Method Under an Unknown Noise Type](#6c4-boundaries-of-the-method-under-an-unknown-noise-type)
  - [6c.5 Label-Free Closed-Loop Cleaning: From "Can Detect" to "Cleaning Actually Works"](#6c5-label-free-closed-loop-cleaning-from-can-detect-to-cleaning-actually-works)
  - [6c.6 Wild-Noise Closed-Loop Cleaning: Negative Downstream Gain, for a Different Reason Than Injected Noise](#6c6-wild-noise-closed-loop-cleaning-negative-downstream-gain-for-a-different-reason-than-injected-noise)
...
The three threads are not simply sequential. Thread 2 is the methodological foundation for Thread 3 — without first knowing what the signatures look like, there is no basis for designing a label-free scorer. But Thread 2's central mechanistic finding (the "direction-reversal trap" in Section 5.2 and [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4) shows precisely why Thread 2 and Thread 3 must be discussed separately: high *supervised* detection accuracy (e.g. `template`'s in-domain AUC of 0.999) does not imply an equally accurate *label-free* scheme exists (the same type scores only 0.522 under `iforest`). Thread 1 is independent of both — no matter how detectable or how mature the label-free scheme, cleaning a noise type that is essentially harmless downstream (e.g. `garbled`'s zero downstream gain after closed-loop cleaning, see [06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.2.1) delivers limited real value.
...
The division of labour with this report is clear: those works mostly rely on an **external scorer** (another LLM, embedding similarity, or a heuristic rule) to judge quality, whereas this report relies only on **the traces the model itself leaves during training**, with no external judge introduced. The two are not in conflict — [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Sections 6b.6 and 6b.8 show that the static text-similarity feature `text_nn_sim` (belonging to the external-signal side) is stronger than the entire training-dynamics feature set for exact duplication and topic mismatch, and completely useless for garbled text.
...
- **[Thread 2]** **Report per noise mechanism**, rather than a single aggregate number. [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4 explains why pooling the report would hide phenomena as important as direction reversal.
- **[Thread 1]** **Harm ranking does not depend on detection accuracy — it goes straight to downstream evaluation.** Most of the literature above stops at "ranking quality" (detection-side metrics like AUC / P@10%). This report additionally runs 7 downstream benchmarks separately for each noise type ([06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.1), splitting "is this noise type easy to detect" and "does this noise type actually cause harm" into two independent questions.
- **[Thread 3]** **It has to go all the way to downstream evaluation.** Ranking quality (AUC / P@10%) does not equal cleaning benefit; [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5 uses real retraining and evaluation to show the two can decouple, or even go in opposite directions.
...
- No selecting feature subsets or tuning scorer hyperparameters by label — [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.8's single-feature ablation shows that choosing features per noise type would leave another 2-5 points of label-free AUC on the table, but that headroom is unreachable in production.
- No using labels to decide which scorer to apply. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5.2 quantifies what this costs: on template, the scorer that points the wrong way leaves downstream performance *worse* than not cleaning at all.

The sole exception is the **supervised AUC in [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Sections 6b.1 and 6b.2**, which deliberately trains a random forest on the labels. Those sections are not a production proposal but a **reference ceiling for the signal**: if even a supervised model cannot separate a noise type, a label-free one certainly cannot. Every supervised number in the report is explicitly marked as such.
...
Every experiment uses Qwen2.5-3B-Instruct + LoRA (r=32) for 5 epochs. The base dataset used to be dolly-15k (instruction-following) alone; `triviaqa-ratio10` (a single factual-QA task, see [06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.3) is now a second task domain, still in training (see Section 3.6). No cross-validation has been run over model scale, LoRA rank, or epoch count.

This premise bears on two classes of conclusion very differently:

- **Detection-capability conclusions** (most sections in [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type)/[06c](#6c-label-free-separability-precision-boundaries-and-real-benefit)) are relatively robust. They rest on the mechanism that noisy and clean samples separate in training dynamics, and the ranking is consistent across the 10% and 5% noise ratios ([06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.3) — ratio cross-validation supplies one layer of robustness evidence, though it is no substitute for cross-validating over model scale.
- **Downstream-harm conclusions** ([06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.1 and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5) are affected far more. On the dolly task domain, the 7-benchmark averages cluster in a narrow 0.42-0.44 band; the downstream harm of noise is small to begin with. Larger models, longer training, or an evaluation set more sensitive to noise could plausibly widen these differences. **The conclusion "this noise type does nearly zero downstream harm" holds only at the current scale and the current task domain** — [06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.3's preliminary evidence shows that on triviaqa, a single homogeneous task, the same ratio of `wrong_answer` noise causes an observable drop on several downstream benchmarks simultaneously, and the harm is no longer averaged away. Whether harm gets masked turns out to depend on task-domain diversity too, not on model scale alone.
...
|---|---|---|---|
| **Full coverage** | 19 | All training samples (14,611+) | Closed-loop cleaning ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5), i.e. what production can actually reach |
| **Diagnostic subsample** | 37 | ~900-1200 rows per dataset | Most AUCs in [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type)/[06c](#6c-label-free-separability-precision-boundaries-and-real-benefit), most sections |
...
- **Closed-loop retraining covers three cases**: `garbled` and `template` (both single-type, [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Sections 6c.5.1/6c.5.2) and `mixed` (the `pooled` scorer, Section 6c.5.3) — training and evaluation are done for all three.
- **The wild-noise closed loop is also done**: `oasst-wild` ([06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Sections 6b.9/6b.10, see below), with `cleaning_loop_targeted_wild`/`cleaning_loop_random_wild` training and all 7 downstream benchmarks complete.
- **Not done**: closed loops for near_duplicate / keyword (detection is weak for both), and multi-seed repeats (so the downstream gains in [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5 and the oasst-wild closed loop carry no variance estimate).
...
**`mixed`'s per-subtype counts fluctuate.** `data.py:175` injects each of the 7 subtypes independently at `ratio/len(types)` = 0.10/7 ≈ 1.43%, reseeding as `seed + offset` each time. The theoretical count per type is 14611 × 0.0143 ≈ 209; the measured counts range from 191 to 211 (near_duplicate 211, template 206, truncation 204, keyword 197, unrelated 197, garbled 194, duplicate 191), the deviation coming from independent sampling occasionally hitting the same row twice, with the later draw overwriting the earlier one. Total noise is 1400 rows, 9.4% of 14,819. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4's per-type slicing is built on these labels.

9 datasets: `clean` (no injection, used as the downstream comparison baseline), 7 single-type datasets, and `mixed`. Two noise ratios, `dolly-ratio10` (10%) and `dolly-ratio5` (5%), the latter used to confirm the conclusions are not an artifact of one particular ratio ([06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.3).
...
This coverage gap runs through the whole report and is the easiest trap in reading the numbers: **13 of the 37 features have only 12.5% coverage**, and since `dropna` requires every feature to be non-null, any analysis using the full feature set effectively runs on only about 900-1200 rows. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.2 opens with an explanation of this gap, and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4 reports both the `full_diag` (37 features, ~919 rows) and `full_coverage` (19 features, all 14,819 rows) populations side by side.
...
4. **Static text features** (`textsim.text_nn_sim`): not dependent on training, computed directly from `train.jsonl` as each sample's text similarity to its nearest neighbour. This is the only feature with 100% coverage that needs no GPU; both [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3's ablation and Section 6c.4's unlabeled baseline use it.
...
|---|---|---|
| In-domain detection difficulty ([06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1) | `unsupervised` | IsolationForest and MAD z-score are fit **independently per dataset**, never merged across datasets (otherwise garbled's extreme magnitude would inflate template's outlier baseline) |
...
| Cleaning-precision lift ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.1) | `precision_lift` | P@10% ÷ random baseline |
| Direction reversal ([06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4) | `memorization` | Fixed sign rule, **never refit per dataset** |
...
**Cross-type transfer and `mixed`**: `cross_type` explicitly skips `mixed` in code (`if ds in ('clean','mixed'): continue`), so it cannot answer the "mixed stream" question. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4 fills this gap with a separate routine, `analyze.py::transfer_to_mixed()`.

Most of this step's analyses (in-domain detection, cross-type/cross-ratio transfer, direction reversal, early detection, feature attribution) primarily serve Thread 2; `precision_lift` ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.1) is a deployment-facing metric that directly serves Thread 3.
...
**Only full-coverage features are used.** Scoring targets the **full training set**, not the diagnostic subsample — otherwise most real noise would fall outside the candidate pool to begin with, making removal pointless. The cost is being restricted to the 19 features that are non-null across the whole dataset, excluding every token-level diagnostic and `cos_global_*`. This is the source of what [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.2 calls "reported AUC and production precision don't always agree."

**Three scorers, corresponding to three noise hypotheses** (see [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4 and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4.5 for detail): `iforest` (noise = outlier, undirected), `memo_signed` (noise = anomalously easy to learn, fixed negative sign), and `pooled` (the three legs standardized and combined by per-sample maximum, for settings where the noise composition is unknown).

**A matched random-removal control is mandatory.** Removing 10% of the samples reduces the training set by 10% regardless of whether the removal is smart; comparing only against the "uncleaned baseline" cannot separate "cleaning benefit" from "data-quantity loss." So every `clean` run produces both `train_targeted.jsonl` (score-based removal) and `train_random.jsonl` (same `n_drop`, same seed, random removal), and both get retrained. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5's four-way comparison (clean baseline / uncleaned / targeted removal / random removal) comes from this.
...
- **12.5% subsample coverage** makes token-level features unavailable at full scale, and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3's ablation shows these features happen to be near_duplicate's only effective signal source — that type's reported AUC is optimistic relative to what production can reach.
- **A single base model at a single scale** (Qwen2.5-3B + LoRA r=32, 5 epochs). The downstream harm from noise is small to begin with ([06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.1: 7-benchmark averages cluster in a 0.42-0.44 band); larger models or longer training could plausibly widen the gap.
- **All 7 noise types are programmatically injected**, with known, consistent perturbations. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4's "unseen types" are still one of these 7 (merely unseen by the detector pool); genuinely wild noise is outside this study's scope.
- **Closed-loop retraining does not cover near_duplicate or keyword**—both have weak detection signal to begin with, making any closed-loop gain hard to interpret; the closed loops that are complete cover garbled, template, mixed ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5) and oasst-wild ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.6).
---
...
This distinction also foreshadows the severity of the tension noted in Section 4.3: the "small-loss criterion" from the noisy-label literature (low loss = trustworthy) **systematically protects** Class B noise. [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4 validates this with data, and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5.2 quantifies its cost in a real cleaning action.
...
**So the theoretical prediction is that both types' detection AUC will be markedly lower than Class A and B**, and this ceiling is not "the wrong scoring method was chosen" — the signal simply does not exist in training dynamics. [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3's ablation confirms this prediction: even adding token-level diagnostics (unavailable in production), near_duplicate only reaches 0.641, and keyword sits at 0.50-0.59 across all three conditions.

### 5.3 The fundamental difficulty under label-free scoring: direction is unknowable

Combining Section 5.2's conclusion with [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1.1's label-free constraint produces a structural difficulty:
...
This feature's existence forces the report to separate two questions: **"can training dynamics detect noise"** vs. **"can the pipeline as a whole detect noise."** [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.6's attribution analysis and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3's ablation exist specifically to answer the first question — if a type's detection signal comes 90%+ from `text_nn_sim`, that type cannot serve as evidence that "the training-dynamics method works."
...
|---|---|---|
| Class A (garbled/unrelated/truncation) outlier detection works | Conflicts with the statistical structure of language, hard to fit | [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1: garbled AUC 0.998 (supervised) / 0.936 (label-free iforest) |
| Class B (template/duplicate) outlier detection fails or reverses | Hyper-typical, abnormally low loss | [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4: template label-free iforest only 0.522 (supervised 0.999); [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5.2: iforest cleaning precision 4.04%, below the 9.24% random baseline |
| Class B needs a signed rule to be detected | Explicitly specifying direction | [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4: `memo_signed` pulls template up to 0.925 |
| Class C (near_duplicate/keyword) is hard to detect | The signal does not live in training dynamics | [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1: supervised AUC only 0.577 / 0.674; [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3: adding token diagnostics does not fix it |
| Detectors need per-type calibration, but not per-ratio calibration | The noise mechanism is orthogonal to the ratio | [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.2 (cross-type loss of 17-20 points) vs. Section 6b.3 (near-zero cross-ratio loss) |

**Thread 3: detectability — deployment-strategy predictions and validation**

| Prediction | Mechanism | Validation |
|---|---|---|
| An unknown type forces running in parallel, lowering the precision ceiling | Direction is unknowable | [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.4.5: `pooled` P@10% 0.321, higher than any single method but not high in absolute terms |
| Ranking quality does not equal downstream benefit (spans Thread 1/Thread 3) | Depends on whether the noise is actually harmful | [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5.1: garbled reaches 5.7× precision but zero downstream gain |
...
|---|---|---|---|
| [06a-harm-ranking.md](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) | Thread 1: harm ranking | Which noise type hurts training the most | 6a.1, 6a.2.1-6a.2.4 |
| [06b-feature-signatures.md](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) | Thread 2: feature signatures | What do each type's training dynamics / text features look like | 6b.1 (incl. 6b.1.1), 6b.2, 6b.3, 6b.4, 6b.5, 6b.6, 6b.7, 6b.8, 6b.9, 6b.10 |
| [06c-detectability.md](#6c-label-free-separability-precision-boundaries-and-real-benefit) | Thread 3: detectability | Can it be separated label-free, how accurate, does cleaning actually pay off | 6c.1, 6c.2, 6c.3.1-6c.3.4, 6c.4.1-6c.4.5, 6c.5.1-6c.5.3, 6c.6 |
...
**The deployment decision driven by this finding** (a single feature replacing a mixed scheme) belongs to Thread 3 — see the feature-group ablation experiment in [06c-detectability.md](#6c-label-free-separability-precision-boundaries-and-real-benefit) Sections 6c.3.1-6c.3.2.
...
Earlier sections separately discuss each type's detection difficulty, feature attribution, and direction-reversal behavior (full detail in [06b-feature-signatures.md](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type)), but nowhere collect "which method should be used for this type, what raw data does that method depend on, and is all of that data actually necessary" in one place. This section is that summary; Section 6c.3 below upgrades the "necessity" judgment from qualitative attribution to quantitative ablation evidence.
...
|---|---|---|
| `full_diag` | All numeric columns (including token diagnostics, `cos_global_*`) | No — this is exactly the raw `unsupervised.csv` result cited in earlier sections (throughout [06b-feature-signatures.md](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) and Section 6c.1 of this file) |
...
**Method and feature list** (`cleaning_loop.py::build`): unlike [06b-feature-signatures.md](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1.1's "in-domain detection AUC," which uses 37 features computed only on the ~900-1000-row diagnostic subsample, scoring here must happen on the **full training set** (otherwise most true noise samples would never even be candidates for removal, making cleaning meaningless), so the feature subset used is "every feature that is non-null for every row in this dataset," excluding all token-level diagnostic features (subsampled, heavily missing) and `cos_global_*` (compared within the same optimizer-step window against other samples, structurally inconvenient to reuse for per-sample scoring), leaving 19 features with 100% coverage:
...
**Core finding one: cleaning produces a genuinely positive downstream gain for the first time, under the correct scorer, and the gain is concentrated exactly where the real damage was.** After `memo_signed` cleaning, GSM8K rises from 0.4602 back to **0.5231**, recovering **70.3%** of the 8.95-point "uncleaned → clean" gap; the 7-benchmark average rises from 0.4260 to 0.4356, recovering **74.4%** of its gap. This lines up perfectly with the diagnosis in [06a-harm-ranking.md](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.1: template's real harm falls almost entirely on GSM8K, so the cleaning benefit shows up almost entirely there too, while the other 6 benchmarks stay within ±0.006 (BBH's +0.0185 shouldn't be read on its own, given n=540). This closes the question garbled's negative result left open: **closed-loop cleaning is genuinely effective; garbled showed no gain because it was the wrong type to test on, not because the method doesn't work.**
...
- **[Thread 1]** `triviaqa-ratio10` currently has only the `clean`/`wrong_answer` datasets finished with training + evaluation; `refusal`/`confusable_wrong` are still in the training queue. Section 6a.3's finding that "under a single homogeneous task, noise harm is not diluted away by averaging across benchmarks" currently covers only the `wrong_answer` noise mechanism, and cannot represent the harm ranking across all noise types on QA tasks — once the remaining data lands, the finding's strength, or even its direction, could change.
...
- **[Thread 2]** Explore dedicated features for "mildly perturbed" noise like keyword substitution and near-duplication ([06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.3's ablation confirms the current training-dynamics + text-similarity feature combination carries very weak signal for both, and even adding token-level diagnostics unavailable in production does not solve it).
...
*Direct evidence for Thread 2 (feature signatures) — real text backing [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1's detection-difficulty ranking.*
...
| `user_loss` | 4.94 | 4.08 | Garbling makes even the prompt hard to predict, raising `user_loss` — this is the direct numeric evidence behind [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.6's finding that garbled-detection relies most heavily on `user_loss` |
...
Using a random forest classifier (supervised, see [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.1.1 for convention) on training-dynamics features with 5-fold cross-validation gives an "in-domain detection AUC" per noise type:
...
- The ranking is identical across both noise ratios (10% vs. 5%), and dolly-ratio5 actually scores slightly *higher* than dolly-ratio10 on the three harder types (unrelated/truncation/near_duplicate/keyword) — early evidence that this ranking is stable rather than a coincidence specific to one noise ratio; [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.3's cross-ratio transfer analysis further confirms this.

---

### 8.8 Raw signal: direction reversal as it appears directly in the loss curves

*Direct evidence for Thread 2 (feature signatures) — [06b](#6b-feature-signatures-of-each-noise-type-how-training-dynamics-and-text-features-vary-by-type) Section 6b.4's raw-curve backing for the direction-reversal trap.*
