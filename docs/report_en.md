...
- **Downstream-harm conclusions** ([06a](#6a-downstream-harm-ranking-which-noise-type-hurts-training-outcomes-the-most) Section 6a.1 and [06c](#6c-label-free-separability-precision-boundaries-and-real-benefit) Section 6c.5) are affected far more. The 7-benchmark averages cluster in a narrow 0.42-0.44 band; the downstream harm of noise is small to begin with. Larger models, longer training, or an evaluation set more sensitive to noise could plausibly widen these differences. **The conclusion "this noise type does nearly zero downstream harm" holds only at the current scale.**
