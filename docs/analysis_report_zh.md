...
### 3.14 自然数据信号验证: 控制实验的信号在真实数据上是否保持 (新增)

§2.6 (跨实验发现 6) 引用 dynanoise Phase 6 主张"受控实验的信号方向在自然数据上成立", 但此前这一环只能靠外部引用。本节用 lmsys-chat-1m 真实对话数据 (n=15,404) 与 ratio10 clean 模型验证这个主张是否在本项目的信号/模型组合下也成立 (`scripts/natural_signal_validation.py`, GPU ~2h)。

**方法**: 用 ratio10 的 clean LoRA 模型 (未接触任何噪音) 在 lmsys-chat-1m 的前 20K 样本上做自回归 rollout (去重后 16,919 条 prompt, 有效 15,404 条), 逐样本计算 `loss_mu` (平均 token 损失) / `loss_cv` (变异系数) / `token_loss_top20` (top-20% token 损失占比), 然后测三者的 Spearman 相关。**无标签、无训练**, 纯后验关联测量。

| 信号对 | 本项目结果 | dynanoise Phase 6 | 解读 |
|---|---|---|---|
| token_top20 ↔ loss_mu | **−0.839** | −0.78 | 集中度与难度**反相关** (比文献报告值更强) |
| loss_cv ↔ loss_mu | **−0.861** | (未报告) | 变异系数与难度反相关 |
| token_top20 ↔ loss_cv | **+0.982** | (未报告) | 近乎完全共线 —— 测同一件事 |

> 全部 p < 1e-300 (实质为零), n=15,404。数据: `results/natural_validation.csv`。

**结论 1: 受控实验的信号方向在真实无标签数据上保持。** 这确认了 §2.6 的可迁移性主张 —— dynanoise 与本项目**两个独立实验**在不同模型/数据上都观测到 token 级集中度与样本难度的负相关 (我们的 −0.839 甚至更强, 可能因为更大的 n、不同的基座模型 Qwen2.5-3B, 或多轮对话语料)。这支撑了"信号可部署到真实数据质量监控"这一应用定位。

**结论 2: 集中度 (`token_top20`) 与变异系数 (`loss_cv`) 几乎完全冗余 (ρ=+0.982)。** 两者从不同角度测**同一现象** —— 某些样本有极少数灾难性难 token, 其余样本均匀容易。实用含义: 特征集里**只留一个**。本报告的默认检测特征 (`entropy` / `frac_hard` / `max_token_loss`) 语义上接近集中度但**不包含** `loss_cv`, 所以不存在冗余; §3.12 新增的 `top20_share` 是**比值 (尺度无关)** 而非绝对离散度, 因此与 CV 正交。

**结论 3: 但"信号相关性迁移" ≠ "可以无标签检测噪音"。** 这是 §2.7 (跨实验发现 7, 2026-09-02 新增) 明确区分的细微之处:
- ✅ 刚证实的: Spearman(信号A, 信号B) 在自然数据上的**符号**与受控数据一致;
- ❌ **不能**由此推出: "自然数据上信号A的离群点就是噪音样本"。

为什么? 因为**自然数据无真值**, 我们无法算 AUC。我们只能说"高集中度样本与低损失样本是同一批样本" (通过相关性), 但不知道这批样本是**好** (因为干净且常见而被学会) 还是**坏** (因为可记忆的噪音而被学会)。§3.9/§3.13 测试的无标签检测是**独立的问题**, 且结论是:
- 通用离群检测对表面损坏有效 (garbled 0.955), 对记忆性噪音失效 (template 0.633 → 需带符号规则 → 0.9994);
- 我们刚测得的相关性与**两者均一致**: 表面损坏在集中度上是**离群值** (极高), 记忆性噪音是**过度典型值** (极低, 位于分布中心)。

所以本节验证给出的是**信号发现** (§2.6 / 发现 6) 的外部效度, 而非**无标签检测** (§3.9/§3.13 / 发现 7) 的替代方案。两者互相支撑但不互相取代。
