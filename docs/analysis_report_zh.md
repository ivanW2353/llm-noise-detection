...
- **第 14 节：生产落地**。用上述验证过的方法做免标签闭环清洗，从"能打分排序"推进到"实际剔除并重训"，这是全报告唯一一个产出真实下游对比的实验。
...
**这是本项目目前唯一一个从"打分排序"推进到"实际清洗动作"的实验，也是与第 2-12 节性质不同的一步**：此前所有分析都只验证"能不能分辨噪音样本"（AUC/lift 数字），从未验证"清洗后重新训练是否真的变好"。

本节的实验用的是 `cleaning_loop.py` 的 `iforest` 打分器。它现在是三个选项之一（另两个是 `memo_signed` 和 13.5 节加入的 `pooled`），选它是因为 garbled 是典型的离群型噪音，`iforest` 的"噪音 = 离群"假设正好成立；第 7 节和 13.5 节说明了这个假设在记忆型噪音和混合流上分别怎么失效。
...
这个结果推翻了闭环清洗实验最初的隐含假设（"检测/剔除精度越高 → 下游表现必然越好"），本身是一个有价值的负结果：**清洗是否有下游收益，取决于这种噪音类型本身是否真的会拖累下游表现，而不仅仅取决于检测/剔除精度有多高**。第 10 节已经识别出模板化才是那个"检测容易 + 确有下游危害"两个条件同时满足的类型（GSM8K 上相对干净基线下降 8.95 个百分点），真正值得验证"清洗后是否真的能带来下游提升"的下一个目标应该是模板化，而非 garbled（详见第 15.3 节）。
...
8. **检测器是"窄"而非"垮"，未知噪音需要两条腿并联**：单类型检测器拉到混合噪音流上整体 AUC 掉到 0.563-0.730，但限定"自己那一类 vs 干净"后保持率 0.96-1.23 无实质衰减——覆盖缺口，不是分布失效。真正未见过的噪音类型上（留一法），训练动态检测器池 AUC 0.416-0.935、`text_nn_sim` 0.396-0.974，两者互补且各有低于随机的类型，只有并联取较优才能让 7 类全部高于 0.66（第 13 节）。

### 15.2 局限性

- 下游 benchmark 影响的整体区分度不高（0.42-0.44 区间），可能是当前评测集不够敏感，或者 LoRA 微调规模/训练轮数下噪音的下游影响本就有限；不能排除更大规模训练或更长期训练会放大这些差异。
- 闭环清洗实验目前只在 `garbled@ratio10` 单一噪音类型/单一比例上验证，尚未推广到其他噪音类型（尤其是检测困难的关键词替换、近似重复，或第 10 节确认下游确有危害的模板化）或混合噪音场景；且事后看来，选 garbled 作为首个验证对象本身有设计缺陷——它是检测最容易但下游危害接近于零的类型，天然无法观测到清洗收益。
...
- **打分器已接入（13.5 节的 `pooled`），闭环重训尚未跑**：在 `mixed` 上按 10% 预算剔除后重新训练+评测下游，验证 P@10% = 0.323 这个排序质量能否转化为下游收益。这是目前唯一一个直接面向"生产环境的脏数据是混合的"这一事实的实验设计，也是回答"清洗混合噪音到底值不值"的唯一途径——第 14 节在 garbled 上得到的负结果说明排序质量不能直接推出下游收益。
