...
4. 方法学边界清晰: 有监督检测覆盖全部 7 类但需要标注; 无标签检测只覆盖两端 (表面损坏 + 一致模式), 中间的语义类噪音 (unrelated/keyword/near_duplicate) 在当前特征体系下缺乏无标签解法, 仍需人工标注或半监督方法。

### 7.2 局限

1. 单一模型 (Qwen2.5-3B) + 单一数据集 (dolly-15k) + 单一 LoRA 配置, 未验证跨模型/跨数据集泛化性。
2. 噪音为人工构造, 真实世界的噪音模式可能更微妙、更混合。
3. 尚无"清洗后重训"的直接收益对照 (清洗收益实验正在 GPU 上进行, 结果将在后续更新中补充)。

### 7.3 后续工作

1. 清洗收益对照实验: top-10% by score 清洗 vs 随机丢弃 10%, 对比重训后的下游表现差异 (进行中)。
