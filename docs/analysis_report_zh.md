...
**混合噪音一行与其余各行的性质不同**：其余 7 行都假设噪音类型已知、方法已按该类型校准，mixed 一行则是“成分未知”的情形，第 12 节专门讨论这种情形下方法的边界，12.5 节给出三路并联打分器的实测代价。
...
| mixed | **`pooled` 三路并联，0.725**（P@10% 0.323，`iforest` 单用 0.706/0.268） | 无归因/组内分析数据（`feature_attribution.csv`/`cross_type.csv` 均无 mixed 行） | 三类原始数据都要：全覆盖轨迹（离群侧）+ 带符号轨迹（记忆侧）+ `text_nn_sim`（静态文本侧） | 12.5 节实测：并联比任何单方法精度高，但代价是 near_duplicate 掉到 0.494（略低于随机），且剔除预算被 duplicate/garbled 占据；这是“成分未知”时的推荐方案，不是精度最优方案 |
...
- **优先级高、成本低**：给 `cleaning_loop.py` 增加第三个 `method` 选项（如 `text_sim`），对 duplicate/unrelated 直接用 `text_nn_sim` 的 zscore 打分,预期剔除精度有实质提升,且 `text_nn_sim` 已经是全覆盖特征,不需要新增数据采集。（**已部分落地**：12.5 节加入的 `pooled` 已把 `text_nn_sim` 的 |z| 作为独立一条腿并联进去，`mixed` 上 duplicate 的逐类型 AUC 达 0.946、召回 97.4%；但 `pooled` 是面向"成分未知"的方案，单类型已校准的场景仍值得一个只用 `text_nn_sim` 的纯净选项，精度更高。）
...
1. **两条腿都要有，且并联而非择一。** 训练动态检测器池 + `text_nn_sim` zscore 同时打分，取较高者。这是上表唯一能消掉"低于随机"格子的组合，成本几乎为零（`text_nn_sim` 已是全覆盖特征，不需要新采集）。12.5 节把这条建议接进了 `cleaning_loop.py`，并给出免标签版本的实测代价——精度确有提升，但"消掉所有低于随机格子"这一点在免标签条件下没有完整复现。
...
### 12.5 把建议接进生产代码：`pooled` 打分器

12.4 节第 1 条建议已经落进 `cleaning_loop.py`，作为第三个 `method` 选项 `pooled`。它把三条腿各自标准化后取逐样本最大值：
...
**注意口径差异**：12.3 节的留一法用的是**有监督**检测器（在其他类型的噪音标签上训过 LR/RF），衡量的是"已有检测器碰到新类型"；这里的三条腿全部**免标签**，衡量的是"手上只有一堆无标签的脏数据时能做到什么"。两者数字不可直接比较，但结论指向同一个方向。
...
**第一，`pooled` 赢在精度，不赢在"消掉所有低于随机的格子"。** 它把 P@10% 从 `iforest` 的 0.268 抬到 0.323（相对提升 20%），代价是 near_duplicate 掉到 0.494——比随机低了 0.006。而 `iforest` 单用在 `mixed` 上恰好一格都没低于随机。12.3 节留一法里"并联能消掉所有低于随机格子"那个结论在这里没有完整复现，原因是免标签的 `memo_signed` 腿本身在 5/7 的类型上低于随机（garbled 只有 0.008，它把乱码排到了最干净的一端），并联时这条腿贡献的噪声比有监督检测器池大得多。所以 `pooled` 应当读作"在未知噪音下用精度换取覆盖面"，不是免费的改进。

**第二，`memo_signed` 单独用在混合流上是灾难性的（AUC 0.380，比随机差 0.12）。** 这不是 bug，是它的设计边界：固定符号规则只对被记忆的超典型噪音有效，混合流里 5/7 的类型是普通离群噪音，它会主动把它们**反向排序**。第 6 节说过这条规则在非记忆型噪音上 AUC 必然低于 0.5，这里是它在真实混合场景下的代价——单类型 template 上的 0.531 剔除精度（vs `iforest` 的 0.040）不能外推到混合流。

**第三，剔除预算会被最容易的类型吃掉。** 看剔除的 1482 条的类型构成：`pooled` 捞到了 186 条 duplicate（该类型 97.4% 全被剔除）和 141 条 garbled（72.7%），但 template 只捞到 14 条（6.8%），尽管 template 的逐类型 AUC 是 0.822。原因是预算有限而 duplicate 的分数普遍更高——**逐类型 AUC 好不等于在共享预算下捞得到**。这与第 5 节"AUC 高不代表清洗预算下真的好用"是同一个现象，在混合流里因为类型间要抢预算而更严重。

综合起来，生产建议是分层的：**噪音类型已知且已校准时用单方法**（template 用 `memo_signed`，garbled 用 `iforest`，duplicate/unrelated 直接用 `text_nn_sim`），精度明显更高；**噪音成分未知或混合时用 `pooled`**，它是唯一在所有 7 类上都不至于严重失效的选项，但要接受精度上限约 0.32 且预算会被最显著的类型占据。`mixed` 的完整闭环重训（剔除后重新训练+评测下游）尚未跑，是 14.3 节列出的下一步。
---
...
本节的实验用的是 `cleaning_loop.py` 的 `iforest` 打分器。它现在是三个选项之一（另两个是 `memo_signed` 和 12.5 节加入的 `pooled`），选它是因为 garbled 是典型的离群型噪音，`iforest` 的"噪音 = 离群"假设正好成立；第 6 节和 12.5 节说明了这个假设在记忆型噪音和混合流上分别怎么失效。
...
- 给 `cleaning_loop.py` 增加 `text_sim` 打分方式，用于 duplicate/unrelated（11.3 节），预期比当前混合特征的 iforest 精度更高，且不需要新增数据采集。12.5 节的 `pooled` 已经把 `text_nn_sim` 并联进去并验证了它在 duplicate 上的效果（逐类型 AUC 0.946），剩下的工作是给已校准的单类型场景提供一个不掺其他腿的纯 `text_sim` 选项。
- 探索针对关键词替换、近似重复这类"轻度扰动"噪音的专用特征（11 节消融证实：当前的训练动态+文本相似度特征组合对它们信号都很弱，即使加入生产环境拿不到的 token 级诊断也无法解决）。
- **打分器已接入（12.5 节的 `pooled`），闭环重训尚未跑**：在 `mixed` 上按 10% 预算剔除后重新训练+评测下游，验证 P@10% = 0.323 这个排序质量能否转化为下游收益。这是目前唯一一个直接面向"生产环境的脏数据是混合的"这一事实的实验设计，也是回答"清洗混合噪音到底值不值"的唯一途径——第 13 节在 garbled 上得到的负结果说明排序质量不能直接推出下游收益。
