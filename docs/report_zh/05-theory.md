...
把上述预期整理成可检验的形式，最后一列指向验证它的小节。按三条主线分组展示：前五行是主线2（各噪音类型的指标特征本身及其验证），后两行是主线3（这些特征能否转化为可落地的分离/清洗方案）——同一张表内容不变，只是重新分组呈现，与第 06 章的三文件拆分（[06b](06b-feature-signatures.md)/[06c](06c-detectability.md)）对应。
