...
| `converge_epoch` | loss 首次小于阈值 2.0 的 epoch 序号，若从未达到则记为 5（`analyze.py:52-53`：`(m<2.0).idxmax(axis=1)`，全程未达标则整行赋值为 `len(ep_cols)`） | 模型"学会"这个样本要花多久——异常样本往往要更久，或者反而"学得过快"（见 `converge_epoch` 与记忆型噪音的关系，第 6.6 节） |
...
这张表直接解释了第 6.2 节的检测难度排序：乱码、模板化、截断在文本层面的改动幅度极大（字符替换、整体替换、硬截断），训练动态自然容易留下痕迹；近似重复、关键词替换只做局部同义替换或实体替换，句子结构和大部分用词都不变，这正是它们域内 AUC 全部 7 类中最低（0.674、0.577）的直观原因——也是第 6.8 节"关键词替换找不到稳定主导特征"这一发现在文本层面的根源。
...
| `user_loss` | 4.94 | 4.08 | 乱码连 prompt 部分都变得难预测，`user_loss` 抬高，这也是第 6.8 节里乱码检测器最依赖 `user_loss` 特征的直接数值证据 |
...
- **完全重复**：同样出现反转（噪音组 1.33→0.27，始终低于干净对照组 1.61→0.56），但差距不像模板化那么悬殊——对应第 6.6 节里这一类型 iforest（0.612）和 memo_signed（0.654）两个 AUC 都不算很强、方向反转并不彻底的原始成因。
