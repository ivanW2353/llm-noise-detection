...
- [3. 实验前提](#3-实验前提)
  ...
  - [3.3 单一基座模型、单一规模；任务域已从单一扩展为两个](#33-单一基座模型单一规模任务域已从单一扩展为两个)
...
- [6a. 下游危害排序：什么样的噪音类型对训练效果的影响最大](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大)
  ...
  - [6a.3 triviaqa-ratio10(QA 任务):同质单任务下,噪音危害不再被"7 项平均"稀释](#6a3-triviaqa-ratio10qa-任务同质单任务下噪音危害不再被7-项平均稀释)
...
- [8. 附录：原始数据与指标定义](#8-附录原始数据与指标定义)
  ...
  - [8.5 采集耗时（实测，`keyword@dolly-ratio10`，单卡 NVIDIA RTX PRO 6000 Blackwell Server Edition——即项目前期那张卡）](#85-采集耗时实测keyworddolly-ratio10单卡-nvidia-rtx-pro-6000-blackwell-server-edition即项目前期那张卡)
...
全部实验用 Qwen2.5-3B-Instruct + LoRA（r=32），5 epochs。基座数据集原本只有 dolly-15k(指令跟随),`triviaqa-ratio10`(单一事实问答任务,见 [06a](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大) 第 6a.3 节)是第二个任务域,共四个噪音类型(`clean`/`wrong_answer`/`refusal`/`confusable_wrong`),均已跑完(见第 3.6 节);尚未做模型规模、LoRA 秩、训练轮数的交叉验证。
...
- **第二个任务域已跑完**：`triviaqa-ratio10`（QA，[06a](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大) 第 6a.3 节），四个数据集 `clean`/`wrong_answer`/`refusal`/`confusable_wrong` 的训练、7 项下游评测与 `qa_correctness` 专项评测均已完成。该 tag 只做特征表与逐样本指标表，**不**做 dolly 那一整套交叉类型/迁移/闭环清洗分析，因此它只用于回答"任务同质度与噪音内容如何共同决定下游危害"这一组问题。
...
这张表揭示了一个被"7 项平均"掩盖的真实效应:**GSM8K 上 template 相对 clean 实际下降了 8.95 个百分点(0.5497→0.4602)**,是全部 7 项 benchmark 里唯一一个下降幅度接近个位数百分点的项目;但因为其余 6 项 benchmark 上 template 反而普遍略有上升(MMLU/ARC/Winogrande 均小幅提升),averaged 后只剩下 1.3 个百分点的净下降(0.439→0.426),单看平均值容易误以为"各项 benchmark 都只是小幅波动",实际上是"一项真实受损、六项基本不受影响甚至略有假性提升"的不均衡结构。GSM8K 是数值推理任务,对生成内容的精确格式/连贯性要求最高,这与"模板化噪音让模型学会输出模板化/敷衍式回答"这一假设相符;而 BBH 样本量偏小,其 -4.26 个百分点的下降需要更谨慎看待(详见 [07](#7-结论与-future-work) 第 7.2 节局限性)。
...
`triviaqa-ratio10` 是本报告目前唯一一个非 dolly(指令跟随)任务域的实验:triviaqa 是单一、高度同质的事实问答任务,答案通常是 1-3 个词的实体(见 [04-design.md](#4-实验方案) 第 4.2 节),这与 dolly 多样化的指令类型形成对照。这一节要回答两个问题:6a.1 节"7 项平均掩盖真实危害"这个陷阱,是不是 dolly 任务域的特例?以及——在同一任务、同一噪音比例下,**噪音的"内容"本身能让危害差多少**?本节覆盖该 tag 全部四个数据集(`clean` 与三种 QA 噪音机制),后者正是三种机制设计出来要回答的问题。
...
4. **发现新类型后的增量成本很低。** 一旦某类未知噪音被人工识别出来,为它注入一批样本、训一个专属检测器就把它从 0.416/0.670 这一档拉到 0.96–1.00 那一档(6c.4.2 节的"仅自己那一类"列)。整个流程不需要重训基座,只需要一次带指标落盘的 LoRA 微调(dolly 规模约 3.1 小时,见 8.5 节),因此"持续发现 + 增量补检测器"是比"追求一个通用检测器"更现实的路线。
...
- **[主线1]** `triviaqa-ratio10` 四个数据集已全部完成训练与评测。此处原先的 caveat 是"目前只覆盖 `wrong_answer` 一种机制，补齐后结论强度甚至方向都可能变化"——**数据补齐后这个担心被证实了一半**：6a.3 节"单一同质任务下危害不被平均稀释"对 `wrong_answer` 成立（qa_correctness -36.25 个百分点），但**不能推广到全部 QA 噪音机制**——`confusable_wrong` 的 qa_correctness 反而比干净基线高 0.35 个百分点，`refusal` 只低 3.70 个百分点。也就是说"同质任务放大危害"这一效应本身依赖于噪音内容与任务能力的关系，不是任务同质度单独决定的（详见 6a.3 节）。这提示后续任何"某任务域噪音危害大"的论断都必须绑定具体噪音机制，不能只绑定任务域。
...
- **[主线2]** Token 级诊断指标目前只对 1/8 的样本采样（`diag_subsample=8`），这个采样率本身会稀释信号：仅用真实采到的 12.5% 样本算 `hard_loss_max` 的 AUC，模板化/近似重复分别是 0.920/0.632，而全量（87.5% 中位数填充后）只剩 0.564/0.515——这两类的检测上限被子采样掩盖了一部分，不是信号本身弱；关键词替换则是真实数据 AUC 也只有 0.555，信号本身就弱，提高采样率帮不上忙（原始数字见 [08-appendix.md](#8-附录原始数据与指标定义) 第 8.2 节，重训成本评估见第 8.5 节）。
...
**重要发现（本次探索验证）**：对模板化、近似重复两类，仅用有真实 token 数据的 12.5% 样本算 `hard_loss_max` 的 AUC 分别为 0.920 / 0.632，而全量（87.5% 中位数填充后）AUC 只有 0.564 / 0.515——说明当前 1/8 子采样**显著稀释**了这两类信号；关键词替换则真实数据 AUC 仅 0.555，说明它的瓶颈是信号本身弱，不是采样率问题（详见 8.5 节的重训成本评估）。这一结论已收入 [07-conclusions.md](#7-结论与-future-work) 第 7.2 节局限性（主线2）。
