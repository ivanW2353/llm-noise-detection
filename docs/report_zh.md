...
全部实验用 Qwen2.5-3B-Instruct + LoRA（r=32），5 epochs。基座数据集原本只有 dolly-15k(指令跟随),`triviaqa-ratio10`(单一事实问答任务,见 [06a](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大) 第 6a.3 节)是第二个任务域,仍在训练中(见第 3.6 节),尚未做模型规模、LoRA 秩、训练轮数的交叉验证。
...
- **下游危害结论**（[06a](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大) 第 6a.1 节与 [06c](#6c-能否免标签分离噪音样本精度边界与真实收益) 第 6c.5 节）受影响更大。dolly 任务域 7 项 benchmark 的平均值集中在 0.42-0.44 这个很窄的区间，噪音的下游危害本身就小。不能排除更大模型、更长训练、或对噪音更敏感的评测集会放大这些差异。**"某类噪音下游危害接近零"这个结论只在当前规模、当前任务域下成立**——[06a](#6a-下游危害排序什么样的噪音类型对训练效果的影响最大) 第 6a.3 节的初步证据显示,换到 triviaqa 这种单一同质任务后,同样比例的 `wrong_answer` 噪音在多个下游 benchmark 上同时造成可观测下降,危害不再被平均掩盖,说明"危害是否被掩盖"本身也受任务域多样性影响,不是模型规模单独决定的。
...
`triviaqa-ratio10` 是本报告目前唯一一个非 dolly(指令跟随)任务域的实验:triviaqa 是单一、高度同质的事实问答任务,答案通常是 1-3 个词的实体(见 [04-design.md](#4-实验方案) 第 4.2 节),这与 dolly 多样化的指令类型形成对照。triviaqa 是本报告第一个非 dolly 任务域,这一节要验证的问题是:6a.1 节"7 项平均掩盖真实危害"这个陷阱,是不是 dolly 任务域的特例,还是在结构不同的任务上依然存在。本节目前只覆盖 `clean`/`wrong_answer` 两个数据集,结论范围有限(详见 [07](#7-结论与-future-work) 第 7.2 节局限性)。
...
- **qa_correctness 单项(-36.25pp)比 7 项平均(-5.38pp)更适合作为本任务的主危害指标。** 这和 [6a.1](#6a1-下游任务实际影响噪音真的会拖累模型效果吗) 节"7 项平均掩盖 GSM8K 真实伤害"是同一个陷阱的重演——qa_correctness 是专门针对 triviaqa 任务本身设计的评测(见 [04-design.md](#4-实验方案)),不会被其余通用 benchmark 的"背景噪声"稀释,后续 QA 任务的危害排序应以它为主,7 项平均仅作为与 dolly 结果可比的参照。
...
下面 6 条大致沿着三条主线各自的证据缺口展开：主线1 需要验证当前危害区分度不高是否只是训练规模不够、尚未显现；主线3 需要把已验证有效的单点改动（text_sim 打分、特征收窄）推广并补上方差估计；主线1 与主线3 交叉处的"按危害加权剔除"问题，则是尚未解决的最终关口。
