...
- **Full coverage vs. diagnostic subsample**: most AUCs are computed on the ~900-1200-row diagnostic subsample (37 features), while closed-loop cleaning operates on all 14,611+ rows (19 features). The former is optimistic, especially for near_duplicate.
...
**Data scale.** 14,611 training samples per dataset (`duplicate` has 16,072, since it appends copies), plus a 400-sample held-out set (200 for the reference gradient, 200 for monitoring held-out loss, shared across all 9 datasets). Noise ratios are `dolly-ratio10` = 10% and `dolly-ratio5` = 5%, with all 9 datasets complete at both ratios.
...
The figure above is the 7×7 transfer matrix under `dolly-ratio10`: each row is the noise type the detector was trained on, each column is the noise type it was tested on, and the diagonal reproduces the within-domain AUC from Section 2.
...
**Method**: uses the same `fit_transfer` function as Section 6.3's cross-type transfer (`analyze.py::cross_ratio_transfer`); the only difference is how "source/target" are paired — here source and target are the **same noise type at two different noise ratios** (e.g. `dolly-ratio10→dolly-ratio5` means fitting on `dolly-ratio10/garbled` and evaluating on the corresponding `dolly-ratio5/garbled`), rather than being paired by noise type as in Section 3. Both directions (`dolly-ratio10→dolly-ratio5`, `dolly-ratio5→dolly-ratio10`) are trained independently, and the diagonal (not shown in this table, i.e. "within-tag within-domain AUC") likewise reuses Section 6.2's already-computed within-domain CV result rather than being recomputed.
...
The table below is the complete raw result for every dataset × all 3 methods under `dolly-ratio10` (the main summary table in this section only shows, per dataset, the single row with the highest lift — which can hide the fact that switching methods on the same dataset can flip the conclusion entirely):
...
Taking keyword substitution (the single hardest-to-detect type of the 7) as an example, here is the complete top-8 feature-attribution ranking (`results/dolly-ratio10/feature_attribution.csv`):
...
This table makes concrete exactly why keyword substitution is hard to detect: the top feature `loss_slope` only reaches an importance of 0.020 (compare duplicate's top feature `text_nn_sim` at 0.155 — nearly 8x higher), and its `importance_std` (0.0198) is **almost equal to the importance itself** (0.0201) — meaning that across the 5 cross-validation folds this feature's importance is deeply unstable: one fold shows it as important, another as nearly useless. This isn't a case of one feature "hiding deep" — keyword substitution, which only swaps 1-2 entity words while leaving sentence structure entirely intact, simply doesn't leave a stable, reproducible trace in either training dynamics or text statistics (consistent with the raw text examples in Section 8.6).
...
|---|---|---|---|---|---|
| Duplicate | `text_nn_sim` (text similarity, a data-level feature) | 0.155 | `cos_global_last` | 0.0092 | 17x |
| Unrelated | `text_nn_sim` | 0.084 | `loss_slope` | 0.0102 | 8x |
...
- Garbled is dominated by `user_loss` (0.015) — garbled text causes the response segment's loss to be uniformly elevated.
- Keyword is dominated by `loss_slope` (0.020), but its standard deviation nearly equals its mean — consistent with keyword being the hardest-to-detect type: no single feature contributes stable signal.
- Truncation is dominated by `loss_slope`/`loss_std` (0.029/0.028).
- Near-duplicate is dominated by `hard_loss_max`/`max_token_loss` (0.024/0.024), both token-level hard-example features.
- **Template** is another notable case: its own-AUC is near-perfect (0.999), yet every feature's permutation importance is tiny (the top one, `hard_id_uniq`, is only 0.0019) — when a type is "too easy to detect," redundant/correlated features dilute each other's marginal contribution. This does not indicate unreliable detection; it is the normal behavior of permutation importance on a near-saturated task.
...
**Raw data: broken down per benchmark, not just the mean.** The table below shows `clean` / `template` / `near_duplicate` (dolly-ratio10) accuracy on each of the 7 benchmarks individually (from `results/eval/eval_dolly-ratio10_{dataset}.json`):
...
| Dataset | dolly-ratio10 mean accuracy | dolly-ratio5 mean accuracy |
|---|---|---|
...
- Within that limited spread, **template's dolly-ratio10 downstream accuracy (0.426) is the lowest of all datasets** — even lower than the clean baseline (0.439) — echoing the Section 6.2/6.6/6.8 conclusion that "template is the easiest to detect and the most deeply memorized": the model's overfit memorization of template noise does leave an observable negative trace downstream, making it the only type in this evaluation where "easy to detect" and "actually harmful" corroborate each other.
- **dolly-ratio5's downstream evaluation is now fully complete (9/9 datasets)**, and the results don't fully match dolly-ratio10: the lowest dolly-ratio5 score turns out to be **mixed noise (0.430)**, not template (0.433, tied mid-pack with truncation/keyword) — meaning the conclusion "template causes the most downstream harm" is **not stable across noise ratios**: at the 5% noise ratio, the compound effect of mixing all 7 noise types together drags down downstream performance more visibly. This suggests template's downstream harm may involve a threshold effect (only showing up clearly once the noise ratio is high enough), while mixed noise's downstream harm may be the result of several mild effects stacking together — worth verifying separately in future work whether mixed noise exhibits a synergistic amplification effect.
...
Before asking which data is necessary, a more basic question: **how independent are these 19 full-coverage metrics from each other?** Two conclusions in this report lean on them being highly correlated — Section 6.11.4 uses it to explain why RF is indifferent to dropping any single metric, and Section 6.11.5 uses it to explain why removing features *helps* IF (dimensional dilution). Both were assertions until now, never measured. Script: `analyze.py::feature_correlation()`, output `results/dolly-ratio10/feature_correlation.csv`.
...
| Dataset | mean \|ρ\| | share of pairs \|ρ\|>0.7 | PCA for 90% var. | Effective dims | VIF>10 |
|---|---|---|---|---|---|
...
| **pooled** | **0.315** | **12.3%** | **7** | **5.08** | **10/19** |

**Finding 1: the 19 metrics span only about 5 effective dimensions.** The effective dimensionality (participation ratio, `1/Σλ²`) sits stably between 4.8 and 5.3, PCA needs just 6-7 components for 90% of the variance, and half the metrics exceed VIF 10 (the conventional near-collinearity threshold). This holds consistently across all 8 datasets, so it is not a quirk of one noise type. **"Highly correlated" now has a quantitative form**, and it directly explains Section 6.11.5: the additional 14 dimensions carry almost no new information and act as noise inside an unsupervised outlier distance.

**Finding 2: the redundancy follows source-family boundaries strictly.** Grouping metrics by which raw signal they derive from (loss / grad_norm / cos_ref / text / update), within-family mean \|ρ\| is 0.481 while cross-family is only 0.257 — as expected, since four views of the same loss curve are not four independent measurements. The most redundant pairs are effectively the same quantity:

| Pair | \|ρ\| | Why |
|---|---|---|
| `loss_last` ↔ `loss_min` | **0.999** | over 5 epochs the last value is almost always the minimum |
| `loss_std` ↔ `loss_slope` | **0.994** | for a monotonically falling curve, spread is set by slope |
| `loss_mean` ↔ `loss_rank` | 0.987 | the latter is the former's percentile rank, near-monotone equivalent |
| `grad_norm_mean` ↔ `update_contrib_mean` | 0.987 | the contribution share is itself normalized by gradient norm |

**Finding 3: only a handful are genuinely independent.** Ranked by maximum correlation to any other metric, the most independent are `cos_ref_std` (0.253), `cos_ref_last` (0.413), `text_nn_sim` (0.568) and `grad_norm_cv` (0.637). This matches Section 6.11.5's greedy selection, where the first picks tend to come from different families rather than swapping one statistic for another within a family.

**Methodological implication**: a redesigned collection could keep one of each pair — `loss_last`/`loss_min`, `loss_std`/`loss_slope`, `grad_norm_mean`/`update_contrib_mean` — with no effect on any conclusion. Keeping all of them is historical (features were added first, overlap measured later), not deliberate redundancy.

**An implementation bug this measurement caught**: `analyze.py` used to assign `cos_ref_trend` directly from `cos_ref_slope` (correlation exactly 1.000), leaving the two columns identical across all 133,168 rows — so the "20 features" were really 19. It had already contaminated Section 6.8's attribution table: permutation importance splits one signal arbitrarily across the twin columns, and on keyword `cos_ref_trend` scored +0.0059 (rank 4) while `cos_ref_slope` scored −0.0009. The assignment is removed, and the numbers in this section and in Sections 6.8 and 6.11.4 come from a recomputation after the fix.
...
Both questions can be answered **without retraining anything**: the diagnostic subsample table (`results/dolly-ratio10/per_sample_metrics.csv`) already contains all three raw data categories — full-coverage trajectory features, `text_nn_sim`, and token-level diagnostics — for the same underlying 900-1,200-row subsample population per dataset (coverage differs by category, but the row population overlaps). All that's needed is rerunning `analyze.py::unsupervised_metrics()` with different `features=` subsets, which finishes in seconds on CPU across all datasets. See `analyze.py::feature_group_ablation()`, output at `results/dolly-ratio10/feature_ablation.csv`.
...
| `full_coverage` | The 19 full-coverage features (Sections 8.1 and 8.3; what `cleaning_loop.py` actually uses in production) | **Yes — current production choice** |
...
Both routes run over the **same features and the same samples** (the diagnostic-subsample population), so an RF Δ and an IF Δ for the same metric are directly comparable. Script: `analyze.py::single_feature_ablation()`, output `results/dolly-ratio10/single_feature_ablation.csv` (336 rows = 8 datasets × 21 conditions × 2 routes). Convention: **Δ = ablated AUC − full AUC**; negative means dropping it hurt (the feature was carrying signal), positive means dropping it helped (the feature was diluting the score).
...
Section 6.11.3's leave-one-out answers "what happens if I drop one," which is the wrong question for deciding **what to collect** — correlated features can each be individually removable while the group as a whole is essential, so nineteen "individually useless" features may still be jointly necessary. Answering "what is the minimum" requires going the other way: start from nothing and greedily add whichever feature most improves AUC, recording the whole path. The k-th point is then the best achievable score using k metrics. Script: `analyze.py::minimal_feature_set()`, output `results/dolly-ratio10/minimal_feature_set.csv`.
...
| Dataset | All 19 | k for 95% | k for 99% |
|---|---|---|---|
| garbled | 0.996 | 1 | 3 |
| duplicate | 0.984 | 1 | 6 |
| template | 0.994 | 2 | 3 |
...
A supplementary experiment (`analyze.py::transfer_to_mixed()`, output `results/dolly-ratio10/transfer_to_mixed.csv`) evaluates all 7 single-type detectors against `mixed`. The protocol matches Section 6.3 exactly (one LR and one RF, `StandardScaler` fit on the source only, higher AUC of the two). `mixed` has 14819 rows — 13419 clean, 1400 noisy — and every row carries a `noise_type` label (near_duplicate 211, template 206, truncation 204, keyword 197, unrelated 197, garbled 194, duplicate 191), which makes per-type slicing inside the mixture possible.
...
- `iforest` z-score — the undirected outlier side, 19 full-coverage features;
...
Using `garbled@dolly-ratio10` (14,611 training samples total, true noise fraction 9.999%) as the test case, a purely unsupervised IsolationForest (no noise labels used, 19-dimensional base trajectory features plus `text_nn_sim`, fit independently on the full dataset) scores and removes a 10% budget of samples:

**Method and feature list** (`cleaning_loop.py::build`): unlike Section 6.2.1's "within-domain detection AUC," which uses 37 features and only the ~900-1000-row diagnostic subsample, this step scores and removes across the **entire training set** (otherwise most true noise samples wouldn't even be candidates for removal, defeating the purpose). It therefore switches to "features that are fully non-null on this dataset" — excluding all token-level diagnostic features (subsampled, heavily missing) and `cos_global_*` (compared within the same optimizer-step window against other samples, not structurally convenient to reuse for single-sample scoring) — keeping only 19 features with 100% coverage:
...
**Second, the removal budget is consumed by the most conspicuous types, which happen to be the harmless ones.** Converting `pooled`'s per-type P@10% (`results/dolly-ratio10/pooled_scorer_compare.csv`) into where the 1482 removal slots actually go:
...
(This project's "best existing" is the per-dataset max from `results/dolly-ratio10/unsupervised.csv` and `memorization.csv`; full numbers in `results/dolly-ratio10/external_baselines.csv`.)

**`oasst-wild` (natural noise, full 68,762 rows):**
...
**Key finding: using only the first epoch's loss/gradient norm (`el2n_proxy`/`grand_proxy`) matches or beats this project's current 5-epoch multi-feature detector on several types** — garbled (0.983 vs. 0.936), keyword (0.682 vs. 0.552), near_duplicate (0.657 vs. 0.599), and unrelated (0.771 vs. 0.703) all favor `el2n_proxy`, using only 1/5 of the training epochs. This lines up exactly with Section 6.7's early-detection finding: most of the noise signal is already visible early in training, so the current label-free production pipeline may not need the full 5 epochs to get a usable signal. `oasst-wild` shows the same pattern — `grand_proxy`/`el2n_proxy` ahead, `tracin_proxy`/`loss_rank` behind.

**`tracin_proxy` (i.e., `cos_ref_mean`) alone is the weakest of the four baselines** (0.51-0.61 on dolly-ratio10, 0.604 on oasst-wild), suggesting that this quantity, often cited in the literature as an influence-function "gold standard," has limited value as a standalone detection signal — it is better used as one leg of a combined score (like `pooled`) than as an independent metric.
...
*Residual signal retained = (stratified_auc − 0.5) / (raw_auc − 0.5), the fraction of the raw "above chance" margin that survives length-decile stratification; the full 20 rows (all 19 features + `pooled_detector`) are in `results/oasst-wild/length_confound.csv`.
...
**Conclusion:** a more careful estimate of `oasst-wild`'s label-free detection capability should use stratified_auc (`pooled_detector` ≈ 0.589) rather than raw_auc (0.700) — a substantial part of the latter is rediscovering the known rule "shorter responses score lower," not discriminative power from training dynamics itself.

### 6.16 Wild-noise closed-loop cleaning: a negative downstream result, for a different reason than the injected-noise loops

Section 6.13's three closed loops (garbled/template/mixed) are all built on **injected noise** — the noise label is written by code, the type is known, and "is it actually harmful" can be measured per type in isolation. `oasst-wild` is this report's only closed loop on **natural noise**: the noise label comes from OASST2's human `quality` rating (<0.30 flagged as noise, see `wild_data.py`). Using the `pooled` scorer (the 26-dimensional feature set recorded in `metadata.json`, the max of the `iforest`/`memo_signed`/`text_nn_sim` legs) to drop 10% of the 68,762 training samples by budget, targeted-removal precision reaches 26.56% (random-removal control 9.63%, a lift of about 3.0× over the true noise rate of 8.83%). Both the targeted and random arms were then trained for 5 epochs and evaluated on all 7 downstream benchmarks.

| benchmark | n | unclean<br>(wild) | targeted removal<br>`pooled` | random removal | pooled − unclean | pooled − random |
|---|---|---|---|---|---|---|
...
**Key finding: this is the only one of the four closed loops where random removal beats no cleaning at all, and targeted removal trails both.** Random removal (0.4195) is slightly above the unclean baseline (0.4150), and targeted removal (0.4107) comes in last — the opposite ranking from Section 6.13.2's template result ("correct direction > no cleaning > random drop > wrong direction"). Here the order is **random drop > no cleaning > targeted removal**: the `pooled` scorer shows no positive return on natural noise, and in fact trails the random baseline.

**A per-benchmark breakdown shows the negative result comes almost entirely from mmlu.** Targeted removal on mmlu (0.5754) is 2.87 points below unclean and 4.43 points below random removal — the largest and least symmetric drop of the 7 benchmarks. Meanwhile gsm8k under targeted removal (0.4920) is actually the highest of the three arms, 4.24 points above unclean. Looking at gsm8k alone would suggest "cleaning works," but the 7-benchmark average is dominated by mmlu's drop, showing that **the per-benchmark effect is not consistent, and the aggregate average can mask an internal split** (consistent with Section 6.9's caution that the 7-benchmark average sits in a narrow band and is easily swayed by a single outlier).

**This negative result has a different mechanism than Section 6.13's, and needs to be read together with Section 6.15's length-confound finding rather than mapped onto the "either precondition missing" framework.** Section 6.13's negative results (garbled being harmless, `mixed`'s budget misallocation) both occur in settings where the detection direction itself was not wrong — the cleaned-out material simply had no real harm. Here, by contrast, the `pooled` scorer carries a known contamination: Section 6.15 measured that 44.5% of its "above chance" signal comes from the response-length confound rather than genuine quality judgment. In the natural-noise setting, the human `quality` rating itself correlates with shorter responses (correlation −0.20, see Section 7.3), so `pooled` likely amplifies that correlation and systematically removes a batch of "short but reasonable" responses rather than genuinely low-quality ones — if that misremoved batch happened to matter disproportionately for training on knowledge-dense multiple-choice tasks like mmlu, that would explain why the downstream loss concentrates there rather than spreading evenly across all 7 benchmarks. **This is a hypothesis, not a verified mechanism: this report ran no dedicated ablation isolating how much of the removed budget was length-confound misfires, and that is left as future work.**

**Methodological implication:** Section 6.13's precondition — "cleaning pays off only when the noise is actually harmful and the scorer's direction is correct" — implicitly treats "direction correct" as binary (right or wrong). `oasst-wild` surfaces a new failure mode: a scorer can be "partially correct" (it does achieve a genuine 3.0× targeted-removal lift on natural noise, clearly better than random) while **the signal it captures is entangled with a confound unrelated to true quality (length)**, so that even though the ranking looks usable, the removal action itself still yields a negative return. This suggests that "scorer direction correct" should be split, for natural noise, into two separate checks: "the ranking direction is correct" and "the signal the ranking relies on is free of systematic confounds." Section 6.15's length-stratification method provides a way to check the latter, but this report has not yet verified whether re-scoring and re-cleaning after removing the confound would turn the result positive — that is one of the next steps listed in Section 7.3.
