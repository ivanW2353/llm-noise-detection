...
This distinction also foreshadows the severity of the tension noted in Section 4.3: the "small-loss criterion" from the noisy-label literature (low loss = trustworthy) **systematically protects** Class B noise. [06b](06b-feature-signatures.md) Section 6b.4 validates this with data, and [06c](06c-detectability.md) Section 6c.5.2 quantifies its cost in a real cleaning action.
...
**So the theoretical prediction is that both types' detection AUC will be markedly lower than Class A and B**, and this ceiling is not "the wrong scoring method was chosen" — the signal simply does not exist in training dynamics. [06c](06c-detectability.md) Section 6c.3's ablation confirms this prediction: even adding token-level diagnostics (unavailable in production), near_duplicate only reaches 0.641, and keyword sits at 0.50-0.59 across all three conditions.

### 5.3 The fundamental difficulty under label-free scoring: direction is unknowable

Combining Section 5.2's conclusion with [06b](06b-feature-signatures.md) Section 6b.1.1's label-free constraint produces a structural difficulty:
...
This feature's existence forces the report to separate two questions: **"can training dynamics detect noise"** vs. **"can the pipeline as a whole detect noise."** [06b](06b-feature-signatures.md) Section 6b.6's attribution analysis and [06c](06c-detectability.md) Section 6c.3's ablation exist specifically to answer the first question — if a type's detection signal comes 90%+ from `text_nn_sim`, that type cannot serve as evidence that "the training-dynamics method works."
...
|---|---|---|
| Class A (garbled/unrelated/truncation) outlier detection works | Conflicts with the statistical structure of language, hard to fit | [06b](06b-feature-signatures.md) Section 6b.1: garbled AUC 0.998 (supervised) / 0.936 (label-free iforest) |
| Class B (template/duplicate) outlier detection fails or reverses | Hyper-typical, abnormally low loss | [06b](06b-feature-signatures.md) Section 6b.4: template label-free iforest only 0.522 (supervised 0.999); [06c](06c-detectability.md) Section 6c.5.2: iforest cleaning precision 4.04%, below the 9.24% random baseline |
| Class B needs a signed rule to be detected | Explicitly specifying direction | [06b](06b-feature-signatures.md) Section 6b.4: `memo_signed` pulls template up to 0.925 |
| Class C (near_duplicate/keyword) is hard to detect | The signal does not live in training dynamics | [06b](06b-feature-signatures.md) Section 6b.1: supervised AUC only 0.577 / 0.674; [06c](06c-detectability.md) Section 6c.3: adding token diagnostics does not fix it |
| Detectors need per-type calibration, but not per-ratio calibration | The noise mechanism is orthogonal to the ratio | [06b](06b-feature-signatures.md) Section 6b.2 (cross-type loss of 17-20 points) vs. Section 6b.3 (near-zero cross-ratio loss) |

**Thread 3: detectability — deployment-strategy predictions and validation**

| Prediction | Mechanism | Validation |
|---|---|---|
| An unknown type forces running in parallel, lowering the precision ceiling | Direction is unknowable | [06c](06c-detectability.md) Section 6c.4.5: `pooled` P@10% 0.321, higher than any single method but not high in absolute terms |
| Ranking quality does not equal downstream benefit (spans Thread 1/Thread 3) | Depends on whether the noise is actually harmful | [06c](06c-detectability.md) Section 6c.5.1: garbled reaches 5.7× precision but zero downstream gain |
