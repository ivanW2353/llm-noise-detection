...
（`features.csv` 里 `loss_rank` 记作 0.941 是取了 |AUC-0.5| 的对称值；本报告统一按
"噪音行该指标是否更高"记录方向，以免误读。）
...
7. **因此下游危害必须相对同任务域的干净基线来读**，跨任务域比较绝对分数会把"任务本身
   造成的遗忘"误算成"噪音的危害"。
8. **免标签检测信号较强**（最强单指标 |AUC-0.5| 达 0.44 量级），且答案长度与目标
...
1. **"能检测"到"清洗有收益"这一步完全没有验证过。** 本实验只做了特征表与逐样本
   ...
   特征消融。`loss_rank` 0.941 这个高 AUC **不能推断出清洗会有下游收益**——
